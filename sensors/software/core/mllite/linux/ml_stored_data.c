...
void *inv_malloc(unsigned int size);
...
inv_error_t inv_read_cal(unsigned char **calData, size_t *bytesRead)
{
    ...
    *calData = (unsigned char *)inv_malloc(fsize);
    if (*calData==NULL) {
        MPL_LOGE("Could not allocate buffer of %zu bytes - "
                 "aborting\n", fsize);
    ...
    *bytesRead = fread(*calData, 1, fsize, fp);
    if (*bytesRead != fsize) {
        MPL_LOGE("bytes read (%zu) don't match file size (%zu)\n",
                 *bytesRead, fsize);
        result = INV_ERROR_FILE_READ;
        goto read_cal_end;
    } else {
        MPL_LOGV("Bytes read = %zu", *bytesRead);
    }
...
inv_error_t inv_write_cal(unsigned char *cal, size_t len)
{
    ...
    if (len <= 0) {
        ...
        MPL_LOGV("cal data size to write = %zu", len);
    }
    ...
    if (bytesWritten != len) {
        MPL_LOGE("bytes written (%zu) don't match requested length (%zu)\n",
                 bytesWritten, len);
        result = INV_ERROR_FILE_WRITE;
    } else {
        MPL_LOGV("Bytes written = %zu", bytesWritten);
    }

    ret = lockf(fd, F_ULOCK, 0);
    if (ret != 0) {
        MPL_LOGE("Cannot unlock file \"%s\"\n", MLCAL_FILE);
    }
    fclose(fp);
    return result;
}
...
inv_error_t inv_load_cal_V0(unsigned char *calData, size_t len)
{
    ...
    MPL_LOGD("Entering inv_load_cal_V0\n");
...
inv_error_t inv_store_cal(unsigned char *calData, size_t length)
{
    ...
    MPL_LOGD("Entering inv_store_cal\n");

    inv_get_mpl_state_size(&size);

    MPL_LOGD("inv_get_mpl_state_size() : size=%zu", size);
    ...
    MPL_LOGD("Exiting inv_store_cal\n");
    return INV_SUCCESS;
}
...
inv_error_t inv_store_calibration(void)
{
    ...
    if (!calData) {
        ...
        MPL_LOGV("inv_get_mpl state size = %zu", length);
    }
    ...
    if (result != INV_SUCCESS) {
        ...
        MPL_LOGV("calData from inv_save_mpl_states, size=%zu",
                 strlen((char *)calData));
    }
