...
static void print_data(const sensors_event_t* events, int count)
{
	...
	for (int i = 0; i < count; i++) {
		...
		if (data->type == SENSOR_TYPE_STEP_COUNTER) {
			printf("%13" PRIu64 ", %13d, %13d, ",data->u64.step_counter, 0, 0);
		} else {
		...
		case SENSOR_TYPE_GYROSCOPE:
			printf("%2hhd", data->acceleration.status);
			break;
		// Quaternion + estimated accuracy
		case SENSOR_TYPE_ROTATION_VECTOR:
		case SENSOR_TYPE_GAME_ROTATION_VECTOR:
		case SENSOR_TYPE_GEOMAGNETIC_ROTATION_VECTOR:
			printf("%+13f, %+13f", data->data[3], data->data[4]);
			break;
		// Calibration bias
...
int main(int argc, char *argv[])
{
	...
	fflush(stdout);
	batch_ms = 0;
	while ((opt = getopt(argc, argv, "hlmpb:")) != -1) {
		switch (opt) {
			case 'h':
				show_usage(argv[0]);
				exit(EXIT_SUCCESS);
