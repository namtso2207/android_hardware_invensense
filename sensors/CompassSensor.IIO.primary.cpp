        ...
        mTimestamp(0)
{
    ...
    // init status
    for ( i = 0 ; i < SENSOR_MAG_NUM ; i++ ) {
        mCompassSensors[i].delay = 0;
        mCompassSensors[i].on = false;
    }
...
int CompassSensor::enable(int32_t handle, int en)
{
    ...
    if (handle == ID_RM) {
        mCompassSensors[SENSOR_MAG].on = val;
    }

    if (handle == ID_M) {
        mCompassSensors[SENSOR_MAG_CAL].on = val;
    }

    if (handle == ID_RMW) {
        mCompassSensors[SENSOR_MAG_WAKE].on = val;
    }

    if (handle == ID_MW) {
        mCompassSensors[SENSOR_MAG_CAL_WAKE].on = val;
    }

    enable = 0;
    for (i = 0 ; i < SENSOR_MAG_NUM ; i++) {
        if (mCompassSensors[i].on) {
            enable = 1;
            break;
        }
    }
    ...
    res = write_sysfs_int(compassSysFs[BUFFER_ENABLE], enable);
    if (res) {
...
int CompassSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    if (handle == ID_RM) {
        mCompassSensors[SENSOR_MAG].delay = ns;
    }

    if (handle == ID_M) {
        mCompassSensors[SENSOR_MAG_CAL].delay = ns;
    }

    if (handle == ID_RMW) {
        mCompassSensors[SENSOR_MAG_WAKE].delay = ns;
    }

    if (handle == ID_MW) {
        mCompassSensors[SENSOR_MAG_CAL_WAKE].delay = ns;
    }

    min_delay = ns;
    for (i = 0 ; i < SENSOR_MAG_NUM ; i++) {
        if (mCompassSensors[i].on && min_delay > mCompassSensors[i].delay) {
            min_delay = mCompassSensors[i].delay;
        }
    }

    freq = 1000000000.0 / min_delay;
    freq_int = (int)(freq + 0.5);
    min_delay = 1000000000LL / freq_int;
...
{
    // assume scale and bits are the same for all channels
    const struct inv_iio_buffer_channel *channel = &compassBufferScan.channels[MAG_X_CHANNEL];
    int scale;

    // scale is in Gauss == 100uT, value in 2^15 for full scale
    scale = channel->scale * 100.0 * (double)(1L << (channel->bits - 1)) * (double)(1L << 15);

    return scale;
}

/* get actual rate from driver */
int CompassSensor::getEngineRate()
{
    VFUNC_LOG;
    ...
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:cat %s (%" PRId64 ")",
            compassSysFs[COMPASS_RATE], getTimestamp());
    res = read_sysfs_int(compassSysFs[COMPASS_RATE], &rate);
    if (res) {
        LOGE("HAL:Compass could not read compass rate");
    }
    ...
    if (mEnable) {
        ...
        for (int i = MAG_X_CHANNEL; i <= MAG_Z_CHANNEL; ++i) {
            ...
            if (!channel->is_enabled) {
                ...
                // apply offset only
                data[i - MAG_X_CHANNEL] = raw + channel->offset;
            }
        }
