                    ...
                    pathP(NULL)
{
    ...
    LOGI("HAL:Secondary Chip Id: %s", sensor_name);
    ...
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:cat %s (%lld)",
            compassSysFs.compass_orient, (long long)getTimestamp());
    FILE *fptr;
...
int CompassSensor::enable(int32_t handle, int en)
{
    ...
    if (handle == ID_RM) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                en, compassSysFs.in_magn_enable, (long long)getTimestamp());
        res = write_sysfs_int(compassSysFs.in_magn_enable, en);
    }

    if (handle == ID_M) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                    en, compassSysFs.calib_compass_enable, (long long)getTimestamp());
        res = write_sysfs_int(compassSysFs.calib_compass_enable, en);
    }

    if (handle == ID_RMW) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                en, compassSysFs.in_magn_wake_enable, (long long)getTimestamp());
        res = write_sysfs_int(compassSysFs.in_magn_wake_enable, en);
    }

    if (handle == ID_MW) {
       LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
               en, compassSysFs.calib_compass_wake_enable, (long long)getTimestamp());
...
int CompassSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    if(handle == ID_RM) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %.0f > %s (%lld)",
                1000000000.f / ns, compassSysFs.in_magn_rate, (long long)getTimestamp());
        mDelay = ns;
        if (ns == 0)
            return -1;
    ...
    if(handle == ID_M) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %.0f > %s (%lld)",
                1000000000.f / ns, compassSysFs.calib_compass_rate, (long long)getTimestamp());
        mDelay = ns;
        if (ns == 0)
            return -1;
    ...
    if(handle == ID_RMW) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %.0f > %s (%lld)",
                1000000000.f / ns, compassSysFs.in_magn_wake_rate, (long long)getTimestamp());
        mDelay = ns;
        if (ns == 0)
            return -1;
    ...
    if(handle == ID_MW) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %.0f > %s (%lld)",
                1000000000.f / ns, compassSysFs.calib_compass_wake_rate, (long long)getTimestamp());
        mDelay = ns;
        if (ns == 0)
            return -1;
...
int CompassSensor::turnOffCompassFifo(void)
{
    int res = 0;
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                        0, compassSysFs.compass_fifo_enable, (long long)getTimestamp());
...
int CompassSensor::turnOnCompassFifo(void)
{
    int res = 0;
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                        1, compassSysFs.compass_fifo_enable, (long long)getTimestamp());
...
int CompassSensor::getSensitivity()
{
    ...
    int sensitivity;
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:cat %s (%lld)",
            compassSysFs.compass_scale, (long long)getTimestamp());
...
void CompassSensor::fillList(struct sensor_t *list)
{
    ...
    /* Not overwrite minDelay and maxDelay here as the supported rates
     * depend on which chip's AUX I2C */
    ...
    if (compass) {
        ...
        if(!strcmp(compass, "compass")
                || !strcmp(compass, "INV_AK09915")
                || !strcmp(compass, "AK09915")
                || !strcmp(compass, "ak09915")) {
            list->maxRange = COMPASS_AKM9915_RANGE;
            list->resolution = COMPASS_AKM9915_RESOLUTION;
            list->power = COMPASS_AKM9915_POWER;
            return;
        }
        if(!strcmp(compass, "compass")
