...
bool SensorBase::DEBUG_BATCHING = true;
#else
...
bool SensorBase::CUSTOM_SENSOR = false;
bool SensorBase::SENSOR_1K_SUPPORT = false;
                          ...
                          data_fd(-1)
{
    ...
    property_get("vendor.invn.hal.debug.batching", value, "0");
    if (atoi(value)) {
        DEBUG_BATCHING = true;
    }
    property_get("vendor.invn.hal.custom.sensor", value, "0");
    if (atoi(value)) {
	    CUSTOM_SENSOR= true;
    }
    property_get("vendor.invn.hal.data.1k_support", value, "0");
    if (atoi(value)) {
	    SENSOR_1K_SUPPORT = true;
    }
}
...
int SensorBase::inject_sensor_data(const sensors_event_t* data)
{
    (void)data;
    return 0;
}
