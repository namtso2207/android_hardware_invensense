...
int read_sysfs_float(const char *filename, float *var)
{
    int res = 0;
    int ret;
    FILE *sysfsfp;
...
void dump_dmp_img()
{
    ...
    static char dmp_path[PATH_MAX];

    inv_get_sysfs_path(sysfs_path);
    snprintf(dmp_path, sizeof(dmp_path), "%s%s", sysfs_path, "/dmp_firmware_1");
    LOGI("HAL DEBUG:dump DMP image");
    LOGI("HAL DEBUG:open %s\n", dmp_path);
    read_dmp_img(dmp_path, (char*)"/data/local/read_img1.h");
}
