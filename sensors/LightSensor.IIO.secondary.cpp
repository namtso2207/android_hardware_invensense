...
static struct sensor_t sSensorList[] =
{
     ...
     SENSOR_STRING_TYPE_LIGHT, "", 112000, SENSOR_FLAG_ON_CHANGE_MODE, {}},
     {"Invensense Light - Wakeup", "Invensense", 1,
     SENSORS_LIGHT_WAKEUP_HANDLE,
     SENSOR_TYPE_LIGHT, 10000.0f, 0.009994f, 0.175f, 0, 0, 15,
     SENSOR_STRING_TYPE_LIGHT, "", 112000, SENSOR_FLAG_ON_CHANGE_MODE | SENSOR_FLAG_WAKE_UP, {}},
     {"Invensense Proxmity", "Invensense", 1,
     SENSORS_PROXIMITY_HANDLE,
     SENSOR_TYPE_PROXIMITY, 5.0f, 1.0f, 0.5f, 10000, 0, 15,
     SENSOR_STRING_TYPE_PROXIMITY, "", 112000, SENSOR_FLAG_ON_CHANGE_MODE, {}},
     {"Invensense Proxmity - Wakeup", "Invensense", 1,
     SENSORS_PROXIMITY_WAKEUP_HANDLE,
     SENSOR_TYPE_PROXIMITY, 5.0f, 1.0f, 0.5f, 10000, 0, 15,
     SENSOR_STRING_TYPE_PROXIMITY, "", 112000, SENSOR_FLAG_ON_CHANGE_MODE | SENSOR_FLAG_WAKE_UP, {}},
};
...
int LightSensor::enable(int32_t handle, int en)
{
    ...
    if ((handle == ID_L) || (handle == ID_PR)) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
            en, lightSysFs.light_enable, (long long)getTimestamp());
        res = write_sysfs_int(lightSysFs.light_enable, en);
    }

    if ((handle == ID_LW) || (handle == ID_PRW)) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
            en, lightSysFs.light_wake_enable, (long long)getTimestamp());
...
int LightSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %lld > %s (%lld)",
            (long long)mDelay, lightSysFs.light_rate, (long long)getTimestamp());
    res = write_sysfs_int(lightSysFs.light_rate, mDelay);

    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %lld > %s (%lld)",
            (long long)mDelay, lightSysFs.light_wake_rate, (long long)getTimestamp());
