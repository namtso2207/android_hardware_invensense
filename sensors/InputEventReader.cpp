...
int InputEventCircularReader::fill(int fd)
{
    ...
    if (mFreeSpace) {
        const int nread = read(fd, mHead, mFreeSpace * sizeof(input_event));
    ...
    LOGV_IF(INPUT_EVENT_DEBUG, "DEBUG:%s exit, numEventsRead:%zu\n",
            __PRETTY_FUNCTION__, numEventsRead);
    return numEventsRead;
}

int InputEventCircularReader::readEvent(input_event const** events)
{
    *events = mCurr;
    int available = (mBufferEnd - mBuffer) - mFreeSpace;
