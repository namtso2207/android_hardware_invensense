...
static struct sensor_t sSensorList[] =
{
     ...
     SENSOR_STRING_TYPE_PRESSURE, "", 200, SENSOR_FLAG_CONTINUOUS_MODE | SENSOR_FLAG_ADDITIONAL_INFO, {}},
     {"Invensense Pressure - Wakeup", "Invensense", 1,
     SENSORS_PRESSURE_WAKEUP_HANDLE,
     SENSOR_TYPE_PRESSURE, 1100.0f, 0.009995f, 0.004f, 26315, 0, 90,
     SENSOR_STRING_TYPE_PRESSURE, "", 200, SENSOR_FLAG_CONTINUOUS_MODE | SENSOR_FLAG_WAKE_UP | SENSOR_FLAG_ADDITIONAL_INFO, {}},
};
...
int PressureSensor::enable(int32_t handle, int en)
{
    ...
    if (handle == ID_PS) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                en, pressureSysFs.pressure_enable, (long long)getTimestamp());
        res = write_sysfs_int(pressureSysFs.pressure_enable, en);
    }

    if (handle == ID_PSW) {
        LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %d > %s (%lld)",
                en, pressureSysFs.pressure_wake_enable, (long long)getTimestamp());
...
int PressureSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    mDelay = 1000000000.f / ns;
    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %lld > %s (%lld)",
            (long long)mDelay, pressureSysFs.pressure_rate, (long long)getTimestamp());
    res = write_sysfs_int(pressureSysFs.pressure_rate, mDelay);

    LOGV_IF(SYSFS_VERBOSE, "HAL:sysfs:echo %lld > %s (%lld)",
            (long long)mDelay, pressureSysFs.pressure_wake_rate, (long long)getTimestamp());
...
void PressureSensor::fillList(struct sensor_t *list)
{
    ...
    const char *pressure = "ICP20100";

    if (pressure) {
        if(!strcmp(pressure, "ICP20100")) {
    ...
    LOGE("HAL:unknown pressure id %s -- "
         "params default to icp20100 and might be wrong.",
         pressure);
    list->maxRange = PRESSURE_BMP280_RANGE;
    list->resolution = PRESSURE_BMP280_RESOLUTION;
    list->power = PRESSURE_BMP280_POWER;
